...
def _create_entry(img, question, answer, category):
    if None!=answer:
        answer.pop('image_id')
...
def _load_dataset(dataroot, name, img_id2val, label2ans):
    ...
    if 'test'!=name[:4]: # train, val
        ...
        for question, answer in zip(questions, answers):
            ...
            category = answer['answer_type']
            if CATEGORY is None or CATEGORY == category.lower():
                entries.append(_create_entry(img_id2val[img_id], question, answer, category))
    else: # test2015
        entries = []
...
def tfidf_from_questions(names, dictionary, dataroot='data', target=['vqa']):
